...
class DBService {
    ...
    /**
     * Export as a styled PDF via jsPDF.
     * Renders the problem into a hidden container, captures with html2canvas, then produces the PDF.
     */
    static async _exportAsPdf(row) {
        ...
        html += `
            <div style="border-bottom: 3px solid #6366f1; padding-bottom: 16px; margin-bottom: 24px;">
                ...
                <div style="font-size: 12px; color: #94a3b8; letter-spacing: 0.05em;">DSE 數學題目導出</div>
            </div>
        `;
        ...
        html += `
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 24px; font-size: 13px;">
                ...
                <tr>
                    <td style="padding: 6px 12px; background: #f8fafc; border: 1px solid #e2e8f0; font-weight: 700;">導出時間</td>
                    <td style="padding: 6px 12px; border: 1px solid #e2e8f0;" colspan="3">${exportTime}</td>
                </tr>
            </table>
        `;
        ...
        if (row.type === 'algebra') {
            ...
            // Geometry – show template text with variable placeholders
        ...
        if (row.type === 'geometry' && qd.figure_code) {
            ...
            html += `<div id="pdf-figure-placeholder" style="text-align: center; padding: 12px; border: 1px dashed #cbd5e1; border-radius: 8px; margin-bottom: 16px; min-height: 200px;"></div>`;
        }
        ...
        if (row.type === 'algebra' && qd.solution?.steps?.length) {
            ...
            qd.solution.steps.forEach((s, i) => {
                html += `<div style="padding: 6px 0; border-bottom: 1px solid #f1f5f9;">
                    ...
                    <strong>${this._escHtml(s.description)}</strong>
                    ${s.expression ? `<div style="margin-left:36px; color:#475569; font-family: 'JetBrains Mono', monospace; font-size:13px;">${s.expression}</div>` : ''}
                </div>`;
            });
            if (qd.solution.final_answer) {
                html += `<div style="margin-top: 8px; padding: 10px 14px; background: #eef2ff; border-radius: 8px; font-weight: 700; color: #4338ca;">
                    最終答案：${qd.solution.final_answer}
                </div>`;
            }
            html += `</div>`;
        }
        ...
        if (steps?.length) {
            ...
            steps.forEach((s, i) => {
                ...
                html += `<tr style="background:${bg};"><td style="padding:8px 12px; border:1px solid #e2e8f0;">${this._escHtml(s.description)}</td><td style="padding:8px 12px; border:1px solid #e2e8f0; text-align:center; font-weight:700; color:#6366f1;">${s.marks}${s.markType || ''}</td></tr>`;
            });
            if (qd.marking_scheme.total_marks) {
        ...
        // ── Render the geometry figure into the placeholder if we have JSXGraph code ──
        const figurePlaceholder = container.querySelector('#pdf-figure-placeholder');
        if (figurePlaceholder && row.type === 'geometry' && qd.figure_code) {
            try {
                const figCanvas = document.createElement('div');
                figCanvas.id = 'pdf-jsxgraph-board';
                figCanvas.style.cssText = 'width: 400px; height: 350px; margin: 0 auto;';
                figurePlaceholder.appendChild(figCanvas);
                // Try to execute JSXGraph code
                if (window.JXG) {
                    const board = JXG.JSXGraph.initBoard('pdf-jsxgraph-board', {
                        boundingbox: [-6, 6, 6, -6], axis: true, showCopyright: false, showNavigation: false
                    });
                    try {
                        const fn = new Function('board', qd.figure_code);
                        fn(board);
                    } catch (e) {
                        console.warn('Figure code exec failed for PDF:', e);
                        figurePlaceholder.innerHTML = '<div style="color: #94a3b8; padding: 20px;">圖形未能渲染</div>';
                    }
                } else {
                    figurePlaceholder.innerHTML = '<div style="color: #94a3b8; padding: 20px;">（圖形需在幾何生成器中查看）</div>';
                }
            } catch (e) {
        ...
        // Hide MathJax assistive-mml to prevent doubled/overlapping text in html2canvas capture
        container.querySelectorAll('mjx-assistive-mml').forEach(el => {
            el.style.display = 'none';
        });
        // Also hide any MathJax source text that may still be visible
        container.querySelectorAll('.MathJax_Preview, .MJX_Assistive_MathML').forEach(el => {
            el.style.display = 'none';
        });
        ...
        // ── Capture to canvas then build PDF ──
        try {
